...
def map_lut1_to_cell(lut, cell_type, widths) -> Optional[str]:
    ...
    if cell_type == "c2":
        return (
            f"c2 {lut['name']} (\n"
            f"    .D00(1'b{bit0}), .D01(1'b{bit0}), .D10(1'b{bit1}), .D11(1'b{bit1}),\n"
            f"    .A1({a}), .B1(1'b0), .A0(1'b1), .B0(1'b1),\n"
            f"    .out({lut['output']})\n"
            f");"
        )
...
_SDFFE_RE = re.compile(r"""\\?\$_DFFE_PP0P_\s*([^\s(]+)\s*\(\s*(.*?)\s*\)\s*;""", re.DOTALL)
_DFF_RE   = re.compile(r"""\\?\$_DFF_PP0_\s*([^\s(]+)\s*\(\s*(.*?)\s*\)\s*;""", re.DOTALL)
...
def map_sdffe_to_s2(inst_name: str, ports: Dict[str,str]) -> str:
    ...
    return (
        ...
        f"    .A1({E}), .B1(1'b0), .A0(1'b1), .B0({R}),\n"
        f"    .clr({R}), .clk({C}), .out({Q})\n"
        f");"
    )

def map_dff_to_s1(inst: str, ports: Dict[str,str]) -> Optional[str]:
    C = ports.get('C')
    D = ports.get('D')
    Q = ports.get('Q')
    R = ports.get('R')
    ...
    if not all([C, D, Q, R]):
        print(f"Warning: Missing ports in DFF instance {inst}")
        return None

    return (
        f"s1 {inst} (\n"
        f"    .D00({Q}), .D01(1'b0), .D10({D}), .D11(1'b0),\n"
        f"    .A1(1'b0), .B1(1'b0), .A0(1'b1),\n"
        f"    .clr({R}), .clk({C}), .out({Q})\n"
        f");"
    )
